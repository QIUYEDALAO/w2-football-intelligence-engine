...
import type { ForwardWaitMonitor as ForwardWaitMonitorData, IntelligenceWorkspaceList, PerformanceSummary, TodayRecommendation, WorkspaceMatch, WorkspaceMatchItem } from "../types/intelligenceWorkspace";
...
function ForwardWaitMonitor({ value }: { value?: ForwardWaitMonitorData }) {
  ...
  const items = value ? [
    ...
    { label: "排除计数", detail: `PIT 不可证明 ${value.exclusions.pit_unprovable} · 证据写入缺口 ${value.exclusions.write_gap_count}`, alert: value.exclusions.pit_unprovable > 0 || value.exclusions.write_gap_count > 0 },
