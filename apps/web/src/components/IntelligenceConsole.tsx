...
function CalibratedValidationCenter({ response }: { response: IntelligenceCalibratedValidationResponse }) {
  ...
  return <section className="v41-validation-center" id="secondary-validation-calibrated" aria-labelledby="validation-calibrated-title">
    ...
    <details className="v41-validation-technical"><summary>冻结去留判据</summary><p>前向且 warmup=false 的 KEPT ≥ 300 后，仅用前向非热身行按 market × selection 检查 bias 恒正、FILTERED 正向率低于 KEPT、按 R2/R3 计算的 KEPT |cal_gap| 不劣于 FILTERED；盈亏仅记录。</p><code>{response.samples[0]?.param_version || "PARAM_VERSION_UNAVAILABLE"}</code></details>
  </section>;
}
