    ...
    <div className="app" data-ui="boss-decision-console">
      <header className="topbar">
        ...
        <div className="headline-kpis" aria-label="今日关键决策指标">
          ...
          <div className="headline-kpi pending"><span>待结算</span><strong>{model.ledger.available ? model.ledger.pendingCount : "—"}</strong></div>
      ...
      <section className="risk-strip" aria-label="风险与例外"><strong>风险与例外</strong><p>自动采集当前{model.automaticCollectionPaused ? "暂停" : "运行"}；高风险赛事 {model.riskExceptionCount}；首发待确认 {model.lineupPendingCount}；验证证据待补 {model.ledger.available ? model.ledger.evidenceRepairPendingCount : "投影不可用"}。</p><div className="risk-meta">最后检查 {dateTimeLabel(model.lastCheckedAt)}</div></section>
