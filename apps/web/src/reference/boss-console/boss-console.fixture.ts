...
function item(
  ...
  return {
    ...
    marketLadder: [],
    eventRisk: { level: "unknown", label: "未评估", code: null },
    dataIntegrityRisk: pick || watch
      ? { level: "none", label: "数据身份与证据完整", code: null }
      : { level: "high", label: "数据证据缺失", code: "NOT_READY" },
    modelUncertainty: value.uncertainty == null
      ? { level: "attention", label: "dynamic evaluation 与不确定性待就绪", code: null }
      : { level: "none", label: `EV 标准误 ±${(value.uncertainty * 100).toFixed(1)}%`, code: null },
    collectionRuntimeRisk: {
      level: "attention",
      label: "调度暂停（受控状态）",
      code: null,
    },
...
export const bossConsoleFixture: BossConsoleModel = {
  ...
  eventRiskExceptionCount: 1,
  operationalRiskExceptionCount: 6,
  ...
  decisions: [
    ...
    item({ id: "1494223", priority: "P2", kickoffUtc: "2026-07-26T12:00:00Z", match: "天狼星 vs 哥德堡", status: "pick", recommendation: "小 3.5 @1.98", modelProbability: .62, marketProbability: .55, probabilityDelta: .07, expectedValue: .061, uncertainty: .038, nextAction: "首发公布后", nextDetail: "首发 / 伤停 / 总进球线变化", ledgerCode: "d4c81b2", scorelineProjection: readyScoreline("22be819dec", "f18ca4e1ef", "全部符合：小 3.5", [{ scoreline: "1-1", sampleCount: 1060, primarySettlement: "WIN" }, { scoreline: "1-0", sampleCount: 930, primarySettlement: "WIN" }, { scoreline: "0-1", sampleCount: 850, primarySettlement: "WIN" }]) }),
    item({ id: "1494222", priority: "P3", kickoffUtc: "2026-07-26T12:00:00Z", match: "布洛马波卡纳 vs 哈马比", status: "pick", recommendation: "客队 -1.25 @1.92", modelProbability: .583, marketProbability: .495, probabilityDelta: .088, expectedValue: .094, uncertainty: .052, eventRisk: { level: "high", label: "高：深盘波动", code: "EVENT_RISK_HIGH" }, nextAction: "盘口变动即重评", nextDetail: "盘口 / 首发 / 伤停", ledgerCode: "b37f20a", scorelineProjection: readyScoreline("33cf920dec", "a29db5f2ef", "全部符合：客队 -1.25", [{ scoreline: "0-2", sampleCount: 1010, primarySettlement: "WIN" }, { scoreline: "1-3", sampleCount: 890, primarySettlement: "WIN" }, { scoreline: "0-3", sampleCount: 740, primarySettlement: "WIN" }]) }),
    item({ id: "1494217", priority: "P4", kickoffUtc: "2026-07-26T14:30:00Z", match: "马尔默 vs 埃尔夫斯堡", status: "pick", recommendation: "大 2.5 @1.85", modelProbability: .602, marketProbability: .541, probabilityDelta: .061, expectedValue: .056, uncertainty: .041, nextAction: "赛前60分钟", nextDetail: "首发 / 天气 / 总进球线", ledgerCode: "9ae41dd", scorelineProjection: readyScoreline("44da031dec", "b30ec6a3ef", "全部符合：大 2.5", [{ scoreline: "2-1", sampleCount: 1080, primarySettlement: "WIN" }, { scoreline: "2-2", sampleCount: 920, primarySettlement: "WIN" }, { scoreline: "3-1", sampleCount: 810, primarySettlement: "WIN" }]) }),
    item({ id: "1494220", priority: "P5", kickoffUtc: "2026-07-26T17:00:00Z", match: "赫根 vs AIK Stockholm", status: "pick", recommendation: "小 3.5 @1.91", modelProbability: .624, marketProbability: .567, probabilityDelta: .057, expectedValue: .049, uncertainty: .036, nextAction: "赛前30分钟", nextDetail: "临场总进球线 / 首发", ledgerCode: "1f6de40", scorelineProjection: readyScoreline("55eb142dec", "c41fd7b4ef", "全部符合：小 3.5", [{ scoreline: "1-1", sampleCount: 1110, primarySettlement: "WIN" }, { scoreline: "1-0", sampleCount: 900, primarySettlement: "WIN" }, { scoreline: "0-1", sampleCount: 820, primarySettlement: "WIN" }]) }),
    item({ id: "1494224", priority: "W1", kickoffUtc: "2026-07-23T17:00:00Z", match: "北雪平 vs 瓦尔贝里", status: "watch", recommendation: "优势不足 · 暂不选方向", modelProbability: .514, marketProbability: .499, probabilityDelta: .015, expectedValue: .008, uncertainty: .049 }),
    item({ id: "1494221", priority: "W2", kickoffUtc: "2026-07-25T15:30:00Z", match: "米亚尔比 vs 厄斯特松德", status: "watch", recommendation: "模型优势不足", modelProbability: .526, marketProbability: .504, probabilityDelta: .022, expectedValue: .013, uncertainty: .042 }),
    item({ id: "1494219", priority: "W3", kickoffUtc: "2026-07-26T14:00:00Z", match: "卡尔马 vs 哈尔姆斯塔德", status: "watch", recommendation: "NO_EDGE · 不操作", modelProbability: .498, marketProbability: .492, probabilityDelta: .006, expectedValue: .002, uncertainty: .039, nextAction: "等待临场", nextDetail: "新报价或阵容变化" }),
    ...[
