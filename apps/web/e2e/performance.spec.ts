...
test("legacy performance route is no longer a second public dashboard", async ({ page }) => {
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    if (new URL(route.request().url()).pathname !== "/v1/dashboard/intelligence-workspace/list") {
  ...
  expect(new Set(requested)).toEqual(new Set(["/v1/dashboard/intelligence-workspace/list"]));
