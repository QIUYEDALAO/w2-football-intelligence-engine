...
All indexes are built with CREATE INDEX CONCURRENTLY (inside an autocommit
block) so the staging tables keep accepting writes during the migration — no
lock on ``outcome_ledger`` / ``dynamic_prematch_evaluations``.
"""
...
def upgrade() -> None:
    with op.get_context().autocommit_block():
        ...
        op.execute(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_outcome_ledger_capture_checkpoint "
        ...
        op.execute(
            ...
            "ON dynamic_prematch_evaluations (official_funnel_eligible, original_state)"
        )
        # funnel 的 row_number 窗口：排序键与窗口 partition/order 对齐，避免每次
        # 对 12k 行 evaluations 全量排序。
        op.execute(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS "
            "ix_dynamic_prematch_evaluation_funnel_window "
