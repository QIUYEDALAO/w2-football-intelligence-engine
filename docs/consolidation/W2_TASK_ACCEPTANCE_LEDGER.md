...
- 当前状态为 `IMPLEMENTATION_COMPLETE_STAGING_PENDING`。下一步仅允许单一代码 PR 通过 verify/staging-parity/predeploy-e2e 后合并，部署最新 main，并在真实 kickoff 前执行一次不回填时间的 append-only controlled capture，再完成 L1/L2 与 denominator/三轨安全验收。
