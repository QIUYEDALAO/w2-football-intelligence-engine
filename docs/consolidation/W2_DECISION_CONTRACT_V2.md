...
## 运行状态附录：2026-07-15 staging rebaseline

- 当前 staging 运行 `3d67a769704e544fb544979c96cf3162328d090c`；API、Web、worker、scheduler 同 SHA，production 未部署。
- Snapshot v2 的 staging 完整性验收通过。40 场批量审计中 80 个 snapshot 均可验证完整性；READY=2，INSUFFICIENT=78。INSUFFICIENT 不得通过 legacy fallback、降低 provenance 或修改 gate 伪装为 READY。
- Corrected Strict AH 证据为 0/35、0/100，保持 `ACCUMULATING`。没有实际 candidate 不是失败，也不得借此开启可见 AH、`RECOMMEND` 或 lock。
- staging `/ops` 已启用 Bearer service credential 与基于实际 Compose bridge 的 CIDR allowlist。本文只记录启用事实与非敏感 CIDR，不记录 credential。
- 初次冷缓存并发出现 7 次瞬时 504，当前为开放性能 WARN。后续只读诊断必须先区分 API 计算、数据库、缓存击穿与 nginx timeout；不得先增加 timeout，更不得改变决策门规避性能问题。

## 运行状态附录：2026-07-15 validation correctness recovery（部署前）

- #295–#298 已合入 `main@51baa80fc4a8878b6a76e3782ff6586f1f39f269`；staging 仍运行 `3d67a769704e544fb544979c96cf3162328d090c`，因此本附录只声明代码与部署前门禁完成，不声明 staging 已恢复。
- 表现分母只能消费 canonical outcome projection。规范键为 `fixture_id + market + recommendation_scope + strategy_version`，同键只取最后一个有效赛前 candidate；raw、重复、冲突、身份不完整和非最终记录只保留在 audit 层，不得逐行计入命中率、ROI、联赛表现或 pending denominator。
- legacy historical candidate 只维持历史 VALIDATION/Wide Shadow 显示兼容，强制标记 `compatibility_only=true`、`corrected_evidence=false`；它永远不能进入 Strict 35/100、direction release evidence 或 corrected calibration。
- 冻结 staging 基线的独立 oracle 与新 projection 一致：VALIDATION raw/canonical/audit-only=`43/16/27`，Wide Shadow=`60/22/38`；canonical duplicate、outcome conflict、cross-track contamination 均为 0。
- Dashboard 同 key 冷构建使用 single-flight，ledger performance 使用文件指纹与 result-event 指纹缓存；不同 window 共用同一 projection。该修复未提高 nginx timeout，也未改变推荐门。
- blocker 优先级固定为 fixture/coverage/market/quote/data/provenance/artifact/feature/strict-edge；无盘口时 `MARKET_UNAVAILABLE` 不得被 estimate fallback 覆盖，feature/xG readiness 仍须独立可见。诊断修复不得把 WATCH/NOT_READY 提升为 pick。
- `w2.strict_ah_canary_checker.v1` 只读取 corrected canonical Strict AH evidence。无候选返回 `NO_CORRECTED_STRICT_CANDIDATE_YET` 且退出 0；损坏或缺失输入源 fail closed。冻结基线 candidate=0，Strict 继续从 0/35、0/100 积累。
- 集成验收：`1370 passed, 4 skipped`，Ruff、Mypy、TypeScript、Web build、offline acceptance、tracked-output guard 全部 PASS。`provider_calls=0`、`db_writes=0`、历史 rewrite=0、production 未部署、RECOMMEND/lock/OFFICIAL 均未开启或写入。

## 运行状态附录：2026-07-15 validation correctness recovery（staging 验收）

- staging 已按 API → worker → Web → scheduler 顺序切换到 `c4bcceb5cb777639251e0db91a9c1f54f5b9c87b`；四服务同 SHA、healthy、restart=0、OOM=false，`/health`、`/ready`、`/v1/version`、`/meta.json` 连续通过。production 未部署。
- 冻结与实时 ledger 均得到 VALIDATION raw/canonical/audit-only=`43/16/27`、Wide Shadow=`60/22/38`；canonical duplicate、outcome conflict、identity-aware unmatched、cross-track contamination 均为 0。Dashboard 展示 25 场历史验证推荐，其中 canonical 已结算 16、pending 9；10 HIT、4 MISS、2 PUSH，decisive hit rate 71.43%。
- 同 key 冷并发 1/2/4/8 共 15 请求全部 HTTP 200、无 502/504；每个 key 只有一个 owner，waiter 为 0/1/3/7。暖缓存 225 请求全部 200。冷 p50/p95/max=`93.7203/109.6345/110.398s`，暖 p50/p95/max=`2.4401/18.4901/61.5158s`，因此正确性恢复 PASS，但高延迟继续作为性能 WARN；未提高 nginx timeout。
- 当前 future 批次 40 场、80 个 FME Snapshot：READY=38、INSUFFICIENT=42，integrity invalid=0、semantic invalid=0。第一 blocker 为 `MARKET_UNAVAILABLE` 34 场、`QUOTE_CAPTURE_TIME_MISSING` 6 场；19 场在无市场时仍正确显示 xG READY，没有卡片因诊断修复被意外提升为 pick。
- Strict canary 对冻结与实时 ledger 均返回 `NO_CORRECTED_STRICT_CANDIDATE_YET`，candidate=0、contamination=0。legacy historical performance 仍不是 corrected evidence，Strict 继续从 0/35、0/100 独立积累。
- 安全验收：主动 provider calls=0、scheduler provider delta=0、业务 DB writes=0、历史 rewrite=0、lock=0、OFFICIAL canonical=0、队列=0、Web 5xx=0；ops 鉴权保持 401/401/200。RECOMMEND、threshold、artifact 和 league enablement 均未改变。

## 运行状态附录：2026-07-16 frozen L2 audit recovery（部署前）

- #305–#307 已合入代码目标 `main@5005df4e4873e618399bedaf4f32c9e5bbb2ef8f`；staging 仍运行 `c4bcceb5cb777639251e0db91a9c1f54f5b9c87b`，本附录不声明已部署。
- Boss View L2 的唯一首层数据源为 exact frozen forward capture。L1 必须公开 `audit_capture_hash` 和 `audit_estimate_id`，L2 必须原样携带二者请求有界 `audit-detail`；不得只按 fixture 读取“最新”数据。
- 公共 HTTP 禁止调用完整 Dashboard、FeatureSet、Simulation、raw history 或 FME 重建。`analysis-card` 只能返回冻结兼容内容或 `FROZEN_ANALYSIS_CAPTURE_UNAVAILABLE`；完整重建函数只能由离线/调度入口显式调用。
- 审计响应硬上限为 512 KiB，最多两个 estimate、每个最多 169 个冻结 matrix cell；缓存/single-flight 最多 64 key、TTL 15 分钟。超限 fail closed，不得通过提高 timeout 掩盖。
- v2 snapshot 必须通过 integrity 与 semantics；legacy 只作 historical compatibility，`corrected_evidence=false`；invalid snapshot 可见但不得派生概率。canonical outcome 继续复用既有 projection，严禁重定义 denominator。
- staging 重部署硬回滚条件包括 API OOM/exit 137/restart/oom_kill 增量、原 OOM fixture 5xx、响应超限、L1/L2 identity 不一致、denominator 或三轨污染，以及任何 RECOMMEND、lock、OFFICIAL、历史 rewrite 或 credential exposure。

## 运行状态附录：2026-07-16 frozen L2 audit recovery（staging 回滚）

- `main@2658d37f8ec7e69de5b5737ff37fb8e9cd822c35` 的单次 staging 部署在 L1 公网门失败后已回滚；当前四服务重新运行 `c4bcceb5cb777639251e0db91a9c1f54f5b9c87b`。production 未部署。
- 失败不是 OOM：新 API cgroup oom/oom_kill=0、exit137=0、restart=0，观察 RSS delta约105.3 MiB。阻塞点是 `future` L1 payload约4.04 MB导致12秒 client timeout，同时 `today/next36`暖延迟超过1.5秒目标。
- L1 是 L2 验收的前置门。L1 不通过时不得继续以压力流量测试 L2、不得宣布 frozen audit 已在 staging 恢复，也不得通过增加 nginx/Web timeout规避 payload 与序列化问题。
- 回滚未改变 canonical denominator、三轨、模型或推荐：VALIDATION `43/16/27`、Wide `60/22/38`，duplicate/conflict/unmatched/contamination=0，Strict=0、RECOMMEND=0、lock=0、OFFICIAL=0；provider delta=0、历史 ledger prefix未重写。

## 运行状态附录：2026-07-16 bounded future DayView pagination（部署前）

- DayView L1必须使用显式字段的冻结capture summary，禁止展开raw capture、完整FME snapshot、score matrix、feature/odds snapshot、pricing shadow、provider payload或全ledger记录。
- `/v1/dashboard/day-view`使用稳定cursor分页：默认20、最大50；cursor绑定API SHA、请求窗口、fixture fingerprint、ledger fingerprint和projection版本。snapshot变化必须返回`409 DAYVIEW_CURSOR_STALE`，不得混合两个snapshot。
- `counts`始终表示完整window，`page_counts`只表示当前页；排序在分页前完成。单页不超过512 KiB，单卡不超过24 KiB，超限必须fail closed且保留fixture与审计identity。
- Boss View不得自动获取所有future页、不得回退无界`/v1/dashboard`、不得增加timeout；本地仅缓存首屏，key绑定date/window/API SHA/snapshot ID且不超过1 MiB。分页卡继续以`fixture_id + capture_hash + estimate_id + API SHA`打开冻结L2。
- 本次只改变读取与展示投影，不改变FME数学、Snapshot schema、estimate/model basis、score matrix、denominator、三轨、Strict policy、direction_allowed、推荐门、lock、artifact或联赛范围。

## 运行状态附录：2026-07-16 bounded future DayView pagination（staging 回滚）

- 分页内容契约在真实公网通过：40场两页、union完整、无重复、完整counts与page_counts一致，单页约41 KiB且单卡最大约1.8 KiB；stale cursor正确返回409。
- 冷首屏5.95秒且无timeout/502/504/OOM，但暖请求约1.6–2.0秒、第二页3.87秒，未满足既定1.5秒/3秒门。不得通过提高timeout、减少业务字段或降低门槛掩盖。
- staging已回滚到`c4bcceb5...`，所以当前线上仍是旧无分页release；合并代码不能被描述为已部署。Frozen L2 staging压力验收仍等待L1延迟门通过。
- 回滚前后provider delta=0、queue delta=0、ledger manifest一致、restart/OOM/oom_kill=0；denominator、三轨、推荐、lock、OFFICIAL和production边界不变。

## 运行状态附录：2026-07-16 final observer/L1 acceptance 与 Frozen L2 identity blocker

- Observer Evidence V2 已在 #327 合并，两个实际出口不同的 GitHub-hosted observer 完整 PASS；当前外部主机仅暖连接门失败。正式公网分类为 `ROUTE_SPECIFIC_WARNING`，允许 staging 验收但继续阻断 production。
- `main@461e4973b957981132cfcfd9fc370e0021f8bae2` 曾按 migration → API → worker → Web → scheduler 顺序切换。L1 的 today/next36/future union、两页分页、大小限制、stale cursor、并发和 direct/isolated-nginx 暖延迟均 PASS。
- Frozen L2 的 legacy fixture `1576804` 正确返回 historical compatibility；但当前 L1 为 fixture `1492140` 绑定的 exact capture `720d570b...`、estimate `null` 在 L2 返回 `409 AMBIGUOUS_CAPTURE`。另一个 Snapshot v2 capture 的独立 PASS 不能替代 L1 exact identity。
- 该结果违反既有“L1/L2 capture 和 estimate identity 一致”硬门，已回滚四服务到 `c89555b98cbcf2c41ecf999eefce9f5c0a9627f5`。下一步只允许最小化修复 L1 frozen-capture identity 选择；不得改变 FME 数学、Frozen L2 projection、pagination、threshold、timeout、RECOMMEND、lock 或 OFFICIAL。
