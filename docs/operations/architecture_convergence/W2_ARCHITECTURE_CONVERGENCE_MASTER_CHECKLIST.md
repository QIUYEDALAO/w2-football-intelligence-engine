...
| `scripts/capture_runtime_release_evidence.py` | `MANUAL_OPS` | 发布证据人工 CLI | operator → script | staging manual | 否 | 无 | `KEEP` | E4 |
...
| `scripts/preflight_runtime_writable.py` | `CI_TRANSITIVE` | `test_future_refresh_staging_parity.py` exact import | GitHub CI → Pytest → test → script module | CI | 否 | 无 | `KEEP` | E2/E5 |
...
| `scripts/run_readiness_fault_injection.sh` | `MANUAL_OPS` | 人工 readiness fault-injection CLI | operator → script | staging manual | 否 | 无 | `KEEP` | E4/E5 |
...
| `scripts/run_stage12a_migration_dry_run.py` | `MANUAL_OPS` | 人工 W1 migration dry-run CLI | operator → script | offline | 否 | 无 | `KEEP` | E4/E5 |
...
- `scripts/seed_competition_runtime_authority.py` 改为 `MANUAL_OPS`；0037
  ...
  operator CLI；
- `scripts/run_stage12a_migration_dry_run.py` 改为 `MANUAL_OPS`；
  Stage12A checker 只读取其源码/存在性，没有 Alembic 或 migration 执行链；
- `scripts/preflight_runtime_writable.py` 改为 `CI_TRANSITIVE`，其 exact module
  由 staging-parity Pytest 导入执行；
- `scripts/capture_runtime_release_evidence.py` 与
  `scripts/run_readiness_fault_injection.sh` 改为 `MANUAL_OPS`，两者均没有
  Docker/Compose/systemd/cron/release exact-path 自动执行入口。

分类合同从 GitHub Workflow、全量 Pytest 的 exact import/subprocess、runtime
配置、Docker/Compose/systemd/cron/release 命令和 Alembic script location
重新生成证据集合；同名库函数不再构成同名脚本的执行证据。
...
CHECK_W2_ALL = PASS
CLASSIFICATION_EVIDENCE_CONTRACT = PASS
CHECK_W2_ALL_EXACT_SCRIPT_NODES = 20
CHECK_W2_ALL_EXACT_TOOL_NODES = 2
CONFIGURATION_EXECUTION_SURFACE_CONTRACT = PASS
DELETION_CLOSURE_ABSENCE_AND_REFERENCE = PASS
INVENTORY_CONTRACT_TESTS = 79_PASS
