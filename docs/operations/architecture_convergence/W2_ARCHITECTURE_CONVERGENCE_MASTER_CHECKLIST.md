...
`WRITE_SIDE_IMPLEMENTATION_AUTHORIZED = true` 只授权 Implementation 02–04 的代码实现，
不授权 Provider、scheduler、生产部署或运行采集。02、03、04 继续按照已下发总指令建立
三个 stacked Draft PR，中间不创建状态闭环 PR；全部验收并合并后再统一更新最终权威状态。
