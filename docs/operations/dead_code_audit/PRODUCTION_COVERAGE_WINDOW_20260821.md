...
- Resource-observation correction: the earlier Scheduler readings of about 78–82% without coverage and 83–85% with coverage were short observations under different work phases, not a controlled paired measurement, so they do not establish a 4–6 percentage-point coverage cost. The instrumented Scheduler has subsequently sustained about 84–86% during full plan generation and returns to idle between runs. Coverage overhead remains `NOT_ISOLATED` until a matched non-instrumented baseline can be measured after the window; the active window must not be interrupted to manufacture that comparison. API `/ready` observations of about 0.111 s and 0.140 s have the same non-controlled limitation. All services remained healthy with zero restarts and memory stayed within configured limits.
