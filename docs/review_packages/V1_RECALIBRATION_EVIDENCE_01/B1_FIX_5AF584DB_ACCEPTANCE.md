...
`7024cb18..5af584db` 是从已部署基线到实现提交的累计区间，包含此前已经落地的 A1/A2/A3
证据文件；因此上面的区间级 `--name-status` 会列出超过 6 个文件，不能用来声称
`5af584db` 单提交范围。以下命令单独证明 `5af584db` 本身只改 6 个文件：4 个生产源码文件、
1 个测试文件、1 个治理清单文件：
...
下表 patch SHA 是每个点名文件在已部署 `7024cb18` 与实现完成态 `5af584db` 之间的完整
逐文件差异；它证明交付后文件状态，不把累计区间误称为单提交范围。
