...
.ko, .nums, .ev, .chip .n, .statusline { font-family: ui-monospace, SFMono-Regular, Menlo,
...
def _render_html_report(
    ...
    *,
    ...
    html = (
        ...
        "<footer>"
        f"生成时间：{escape(generated)} · 只读报告 · 数据来自 dashboard payload<br>"
        "盘口走势仅作参照、未验证，不构成方向；"
...
def _recommendation_text(match: dict[str, Any], recommendation: dict[str, Any]) -> str:
    ...
    home = _team_display_name(match.get("home_team_name"), fallback="主队")
    away = _team_display_name(match.get("away_team_name"), fallback="客队")
...
def _team_display_name(value: Any, *, fallback: str) -> str:
    if value is None:
        return fallback
    text = str(value).strip()
    if not text:
        return fallback
    return _TEAM_NAME_CN.get(text, text)
