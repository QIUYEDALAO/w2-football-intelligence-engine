...
LINEUP_FIRST_SEEN_BOOTSTRAP_LEAD_MINUTES = 120
LINEUP_FIRST_SEEN_EMPIRICAL_MIN_SAMPLE = 5
LINEUP_FIRST_SEEN_PLAN_HORIZON = timedelta(hours=4)
...
def empirical_start_lead_minutes(observations: Sequence[int]) -> int:
    usable = sorted(max(int(value), 0) for value in observations)
    if len(usable) < LINEUP_FIRST_SEEN_EMPIRICAL_MIN_SAMPLE:
        return LINEUP_FIRST_SEEN_BOOTSTRAP_LEAD_MINUTES
    ...
    *,
    ...
    if candidate.completed_poll_count == 0:
        target = kickoff - timedelta(
            minutes=empirical_start_lead_minutes(observed_minutes_to_kickoff)
        )
...
class LineupFirstSeenRepository:
    ...
    def append_event(self, payload: Mapping[str, Any]) -> bool:
        ...
        provider_fixture_id = str(payload["provider_fixture_id"])
        with Session(self.engine) as session:
            existing = session.scalar(
                select(LineupFirstSeenEventModel).where(
                    LineupFirstSeenEventModel.provider == provider,
                    LineupFirstSeenEventModel.provider_fixture_id == provider_fixture_id,
                )
            )
            if existing is not None:
                return False
            try:
                session.add(LineupFirstSeenEventModel(**dict(payload)))
                session.commit()
                return True
        ...
        with Session(self.engine) as session:
            return [
                ...
                for value in session.scalars(
                    select(LineupFirstSeenEventModel.minutes_to_kickoff).order_by(
                        LineupFirstSeenEventModel.first_seen_at
                    )
                )
            ]
    ...
    def distribution_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "adaptive_source": (
                ...
                else "BOOTSTRAP_NO_EXTERNAL_TIMING_CLAIM"
            ),
        }
    ...
    def poll_candidates(self, *, now: datetime) -> list[LineupPollCandidate]:
        current = _utc(now)
        horizon = current + LINEUP_FIRST_SEEN_PLAN_HORIZON
        with Session(self.engine) as session:
            identities = list(
                session.scalars(
                    ...
                    .where(
                        ...
                        MatchdayFixtureIdentityModel.kickoff_utc <= horizon,
                    )
            ...
            seen = {
                ...
                for row in session.scalars(
                    select(LineupFirstSeenEventModel).where(
                        LineupFirstSeenEventModel.provider == "api_football"
                    )
                )
            }
            plans = list(
