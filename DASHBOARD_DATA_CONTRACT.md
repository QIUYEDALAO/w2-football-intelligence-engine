...
| `matches[].market_collection.{target_checkpoint,scheduled_at,window_end_at,overdue}` | persisted non-test/non-namespaced odds checkpoint plan | `AVAILABLE_WHEN_SCHEDULED` | `ODDS_PREMATCH` | `NOT_YET_DUE` before the scheduled time; `AWAITING_COLLECTION` inside the due window; the same cause with `overdue=true` only after `window_end_at`; never derived from `readiness.next_eval_at` | `true` |
| `matches[].market_collection.{latest_snapshot_at,latest_snapshot_checkpoint}` | latest linked odds capture that produced persisted market observations | `AVAILABLE_WHEN_OBSERVED` | `ODDS_PREMATCH` | identifies the factual snapshot and checkpoint; an old snapshot remains diagnostic evidence and is not relabelled as a failed collection window | `true` |
...
| `matches[].market_fact.status`, `main_line` | canonical public market-evidence readiness over the Round-3 latest persisted AH/OU snapshot | `AVAILABLE_WHEN_EVIDENCE_EXISTS` | `ODDS_PREMATCH` | exact `READY/INSUFFICIENT`; snapshot age is a factual diagnostic field, while collection failure is determined only by `market_collection` | `true` |
...
| `matches[].market_radar.markets.{ASIAN_HANDICAP,TOTALS}.status` | latest persisted snapshot and exact market inputs | `AVAILABLE` | `ODDS_PREMATCH` | sole public market-evidence authority: `READY` when the persisted diagnostic market is usable, otherwise `INSUFFICIENT`; no fixed-age `STALE` public state | `true` |
...
| `.main_line`, `bookmaker_count`, `prices`, `probabilities`, `latest_snapshot_at`, `quote_age_seconds` | Round-3 latest persisted snapshot | `AVAILABLE_WHEN_EVIDENCE_EXISTS` | `ODDS_PREMATCH` | null/empty when no snapshot; age is always explicit and never silently presented as a live/current quote | `true` |
...
| `matches[].priority_reason_primary`, `priority_reason_secondary[]` | existing intelligence, market, readiness and model relation fields | `AVAILABLE` | relevant source domains | primary is only eligible `MARKET_MOVEMENT/MODEL_DIAGNOSTIC`; data/lineup/collection attention remains secondary and never increments priority counts | `true` |
...
| `matches[].market_radar.markets.*.cross_sectional_comparison_status` | persisted same-time bookmaker pairs | `AVAILABLE` | `ODDS_PREMATCH` | may be available when trend is insufficient; collection-window state does not erase factual diagnostic comparison | `true` |
...
| `matches[].shadow_candidate.authoritative_input_readiness.{quote_freshness_policy_version,quote_age_seconds,quote_max_age_seconds}` | exact candidate quote identity at decision evaluation | `AVAILABLE_WHEN_CANDIDATE_EVALUATED` | `ODDS_PREMATCH` | independent hard gate `w2.quote_freshness.v1`; max age is exactly 1800 seconds; over-age yields `NOT_READY/QUOTE_TOO_OLD_FOR_CANDIDATE` and never changes diagnostic market availability | `true` |
