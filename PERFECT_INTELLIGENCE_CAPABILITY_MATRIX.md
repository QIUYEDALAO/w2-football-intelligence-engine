...
| Seven-state Attention | Frozen precedence, identity/kickoff, real reason codes, affected domains, factual summary, readiness context and next evaluation | `src/w2/dashboard/intelligence.py`, Decision/Data Readiness fields, `tests/unit/test_market_intelligence_projection.py` | `AVAILABLE` | Reuse existing evidence; deterministic projection only |
| Four independent risks | Exact Event/Data/Model/Collection production-shaped dimensions remain independent and fail closed | same sources as Attention | `AVAILABLE` | Reuse exact axes; reject missing/extra/market risk |
...
| History/replay | What was known, what W2 judged (`decision_summary`), reasons/outcomes and card-hash checks | `src/w2/replay/front_door.py`, date navigation | `AVAILABLE_PURE_FUNCTION` | Preserve the existing front-door value against the same DayView |
