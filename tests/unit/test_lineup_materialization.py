...
def test_lineup_materialization_persists_only_the_first_seen_event(monkeypatch) -> None:
    ...
    with Session(engine) as session:
        events = list(session.scalars(select(LineupFirstSeenEventModel)))
    assert len(events) == 1
    assert events[0].provider_fixture_id == "901"
    assert events[0].first_seen_at.replace(tzinfo=UTC) == first_at
    assert events[0].minutes_to_kickoff == 50
    assert events[0].coverage_status == "COMPLETE_BOTH_TEAMS"
...
def test_lineup_materialization_does_not_collect_first_seen_when_disabled(monkeypatch) -> None:
    ...
    with Session(engine) as session:
        assert session.scalar(select(func.count(LineupFirstSeenEventModel.event_id))) == 0
