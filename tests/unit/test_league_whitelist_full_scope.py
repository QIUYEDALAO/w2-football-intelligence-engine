...
def test_scope_excludes_world_cup_tournament_football() -> None:
    assert "world_cup_2026" not in build_scope_summary()["remaining_unaudited_whitelist"]
    assert all(
        item["competition_id"] != "world_cup_2026"
        for item in build_scope_summary()["inventory"]
    )
