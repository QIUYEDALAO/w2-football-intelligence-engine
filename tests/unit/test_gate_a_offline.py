...
def test_authorization_is_independent_scoped_short_lived_and_db_only() -> None:
    authorization = GateARuntimeAuthorization.from_mapping(authorization_payload())
    ...
    for overrides, code in failures:
        with pytest.raises(GateAError, match=code):
            GateARuntimeAuthorization.from_mapping(authorization_payload(**overrides))
...
def test_db_reservation_is_one_shot_fenced_and_atomically_capped(monkeypatch, tmp_path) -> None:
    ...
    authorization = GateARuntimeAuthorization.from_mapping(
        authorization_payload(provider_call_cap=2)
    )
