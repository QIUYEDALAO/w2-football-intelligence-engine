...
def is_html_response(payload: str) -> bool:
...
def main() -> int:
    ...
    try:
        ...
        return 2
    debug = (
        cast(dict[str, Any], dashboard.get("debug"))
        if isinstance(dashboard.get("debug"), dict)
        else {}
    )
    all_rows = (
        cast(list[Any], dashboard.get("all")) if isinstance(dashboard.get("all"), list) else []
    )
    checks = {
    ...
    if not allow_empty_data and dashboard.get("data_profile") == "empty":
        failures.append("dashboard data is empty")
    if args.require_future_fixtures_visible and int(debug.get("future_fixture_count") or 0) <= 0:
        failures.append("future fixtures are not visible")
    if (
        args.require_next_available_date_if_empty
        and not all_rows
    ...
    if is_html_response(ready):
        failures.append("public /ready returned HTML; check nginx SPA fallback misroute")
    if failures:
