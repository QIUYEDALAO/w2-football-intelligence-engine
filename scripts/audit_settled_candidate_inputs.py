...
def audit(path: Path) -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "calibration_status": simulation.get("calibration_status") or capture.get("calibration_status"),
                ...
                "simulation_input_hash": simulation.get("calibration", {}).get("simulation_input_hash"),
                "capture_simulation_input_hash": capture.get("source_artifact_hashes", {}).get("simulation_input_hash"),
                "frozen_input_identity_match": (
    ...
    return {
        ...
        "simulation_status": dict(Counter(
            "READY" if row["lambda_home"] is not None and row["lambda_away"] is not None else "UNAVAILABLE"
            for row in rows
        )),
        "frozen_input_identity_match": dict(Counter(str(row["frozen_input_identity_match"]) for row in rows)),
        "input_readiness": {
            key: dict(Counter(str(row["input_readiness"].get(key)) for row in rows))
            for key in (
                "xg_status", "ratings_used_in_lambda", "squad_value_used_in_lambda",
                "lineups", "h2h_ready", "history_ready", "proxy_elo_excluded",
            )
        },
