#!/usr/bin/env python3
"""Frozen-protocol runner for EV-SE v6. Emits evidence and verifies it.

Protocol: docs/review_packages/EV_SE_DRIFT_V6/PROTOCOL_FROZEN_V6_20260827.md
(commit b74766f9).

The alpha estimates are unchanged from v3 onward. What v6 changes is the population
those windows are built on: v5 drew evaluation epochs from the xG-carrying series,
so an epoch existed only where the target fixture produced xG afterwards and where a
full twenty-row window happened to be available. Production requires neither.
...
PACKAGE = os.path.join(
    os.path.dirname(__file__), "..", "docs", "review_packages", "EV_SE_DRIFT_V6"
)
EVIDENCE = os.path.join(PACKAGE, "EV_SE_DRIFT_V6_EVIDENCE.json")
ALPHA_LEVEL = 0.05
STRATA = 4
MIN_STATES = 30
...
def se0_squared_quantiles() -> dict[str, dict[str, float]]:
    """SE0^2 across the authoritative production epoch set.

    v4 computed this per season and v5 restricted it to epochs whose target fixture
    later carried xG and whose window happened to be full. Both are conditions
    production does not impose, so it now reads from `ev_se_v6_epochs`.
    """
    out: dict[str, dict[str, float]] = {}
    for comp in ("attack", "defence"):
        for cell, states in E.analysis_epochs(comp)["cells"].items():
...
def build() -> dict[str, Any]:
    ...
    for cell, stats in sorted(se0.items()):
        ...
        form[cell] = row
    return {
        "schema_version": "w2.ev_se.drift_v6.evidence.v1",
        "protocol_commit": "b74766f9",
        "history": {
            "v2": "b34eada9", "v3": "e429bd97", "v4": "5a40f448", "v5": "9e4e4723",
        ...
        "confound_diagnostic": {
            "artefact": "EV_SE_DRIFT_V6_CONFOUND.json",
            "note": (
                "held as its own frozen artefact rather than recomputed inline; its "
                "null bootstrap is a fifteen-minute run and --check would carry it "
                "twice. Referenced by SHA-256 under external_artefacts"
            ),
        },
        "parameter_state": {
...
def main() -> int:
    ...
    with open(EVIDENCE, encoding="utf-8") as fh:
        stored = fh.read()
    if stored != text:
        print("EV_SE_DRIFT_V6_EVIDENCE_DIFF", file=sys.stderr)
        return 1
