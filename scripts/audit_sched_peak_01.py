...
`CHECKPOINT_MISSING` 的具体成因是 **单 worker 串行化，加上任务的 Provider 后处理超过剩余窗口/租约**。生产 worker 明确为 `--concurrency=1`。18:30:49Z 调度器一次性成功 claim `14/14` 个 prematch 计划，说明不是没取走，也不是初始 claim 争用；11 个已发出的峰值请求全部 HTTP 200，最大 `{e['provider']['peak_max_elapsed_ms']}ms`，说明不是 Provider 超时。
...
1. **先做有界容量修复：worker concurrency `1 → 2`。** 冻结时间线按 FIFO、并保留观测到的 task/child 切换占用重放，巴甲批次预计在 claim 后 `{e['capacity_option']['brazil_finish_after_claim_seconds']:.3f}s` 完成，距短窗口结束尚有 `{e['capacity_option']['replay_margin_seconds']:.3f}s`；因此 `2` 是这次冻结负载的最小可行并发，不是通用阈值。代价是多一个 Celery 子进程、额外 DB 连接和近似增加一份 worker 工作集；并行请求还会提高瞬时 Provider burst，必须保留现有 tick hard cap 与 quota guard，并在部署 Gate 复测内存、DB 连接和峰值 Provider burst。
